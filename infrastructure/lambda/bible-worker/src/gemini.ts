 ...
 */
function buildSystemPrompt(workInfo: WorkInfo, chapterRange: ChapterRange, customPrompt?: string): string {
 ...
 */
function analyzeError(error: unknown): AnalysisError {
...
function parseAnalysisResult(text: string): AnalysisResult {
  ...
  try {
    ...
    return {
      characters: Array.isArray(parsed.characters) ? parsed.characters.map((c: AnalyzedCharacter) => ({
      ...
      terms: Array.isArray(parsed.terms) ? parsed.terms.map((t: AnalyzedTerm) => ({
        original: t.original || "",
      ...
      events: Array.isArray(parsed.events) ? parsed.events.map((e: AnalyzedEvent) => ({
        title: e.title || "",
