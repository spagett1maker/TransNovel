...
// ─── 동적 섹션 빌더 ───
function buildGlossarySection(context) {
...
function buildCharacterSection(context) {
    if (!context.characters || context.characters.length === 0)
        return "";
    const mainCharacters = context.characters.filter((c) => c.role === "PROTAGONIST" || c.role === "ANTAGONIST");
    const supportingCharacters = context.characters.filter((c) => c.role === "SUPPORTING");
    return `
...
function buildGuideSection(context) {
 ...
 */
function buildSafetyBypassPrompt(context) {
    const glossarySection = buildGlossarySection(context);
    const characterSection = buildCharacterSection(context);
