...
async function processMessage(record) {
    ...
    try {
        ...
        const translationPromise = (0, gemini_1.translateChapter)(contentWithTitle, context, apiKey, 2 // maxRetries (모델당 2회 × 3모델 = 최대 6회, SQS 재시도 3회와 합산)
        );
        const rawTranslated = await Promise.race([
            translationPromise,
            new Promise((_, reject) => {
                setTimeout(() => reject(new Error(`챕터 번역 시간 초과 (${CHAPTER_TIMEOUT_MS / 60000}분)`)), CHAPTER_TIMEOUT_MS);
            }),
        ]);
        // 번역된 제목과 본문 분리
