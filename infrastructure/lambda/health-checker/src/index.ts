...
import { ScheduledHandler, SQSHandler, SQSEvent, SQSRecord } from "aws-lambda";
import { PrismaClient } from "@prisma/client";
import { getSecrets, DatabaseSecrets } from "./secrets";
 ...
 */
export const healthCheckHandler: ScheduledHandler = async () => {
 ...
 */
export const cleanupHandler: ScheduledHandler = async () => {
 ...
 */
export const dlqHandler: SQSHandler = async (event: SQSEvent) => {
 ...
 * Clean up old completed/failed jobs
 */
async function cleanupOldJobs(db: PrismaClient): Promise<{
 ...
 * Generate health metrics
 */
async function generateMetrics(db: PrismaClient): Promise<{
 ...
 * Process DLQ message
 */
async function processDlqMessage(db: PrismaClient, record: SQSRecord): Promise<void> {
  let message: { jobId?: string; chapterId?: string; workId?: string };
